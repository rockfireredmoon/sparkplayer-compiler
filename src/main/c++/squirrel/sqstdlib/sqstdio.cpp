...
SQRESULT sqstd_loadfile(HSQUIRRELVM v,const SQChar *filename,const SQChar *debugpath,SQBool printerror)
{
	...
	if(file){
		...
		if(us == SQ_BYTECODE_STREAM_TAG) { //BYTECODE
			...
			if(SQ_SUCCEEDED(sq_compile(v,func,file,debugpath,printerror))){
				sqstd_fclose(file);
				return SQ_OK;
			}
		}
...
{
	if(SQ_SUCCEEDED(sqstd_loadfile(v,filename,filename,printerror))) {
		sq_push(v,-2);
