...
static SQInteger base_getstackinfos(HSQUIRRELVM v)
{
	...
	if (SQ_SUCCEEDED(sq_stackinfos(v, level, &si)))
	{
		...
		sq_createslot(v, -3);
		sq_pushstring(v, _SC("fid"), -1);
		sq_pushinteger(v, si.funcId);
		sq_createslot(v, -3);
...
static SQInteger base_array(HSQUIRRELVM v)
{
	...
	if(sq_gettop(v) > 2) {
		...
		SQInteger n = tointeger(size);
		if( n >= 0 )
		{
			a = SQArray::Create(_ss(v),n);
		}
		else
		{
			// Negative value means reserve space (i.e. it's a hint)
			// but don't actually put values into it.
			a = SQArray::Create(_ss(v),0);
			a->Reserve(-n);
		}
	}
