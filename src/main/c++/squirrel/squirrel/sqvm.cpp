...
bool SQVM::ARITH_OP(SQUnsignedInteger op,SQObjectPtr &trg,const SQObjectPtr &o1,const SQObjectPtr &o2)
{
	if(sq_isnumeric(o1) && sq_isnumeric(o2)) {
			if((type(o1)==OT_INTEGER) && (type(o2)==OT_INTEGER)) {
				...
				case '-': res = f1 - f2; break;
				case '/': if(f2 == 0.0f) { Raise_Error(_SC("division by zero")); return false; }
					res = f1 / f2;
					break;
				case '*': res = f1 * f2; break;
				case '%': if(f2 == 0.0f) { Raise_Error(_SC("division by zero")); return false; }
...
// Removed in 2.1.2 ?? restored in 2.2.4
#if SQ_NATIVE_DEBUGHOOK
	if ( _debughook )
		CallDebugHook(_SC('c'));
#else
...
void SQVM::CallDebugHook(SQInteger type,SQInteger forcedline)
{
	...
	const SQChar *srcName = type(func->_sourcename) == OT_STRING ? _stringval(func->_sourcename) : 0;
	const SQChar *name = type(func->_name) == OT_STRING ? _stringval(func->_name) : 0;
	_debughook(this, type, srcName, forcedline?forcedline:func->GetLine(ci->_ip), name);
#else
