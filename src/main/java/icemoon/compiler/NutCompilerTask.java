...
public class NutCompilerTask extends AbstractCompilerTask {
	...
	public void execute() throws BuildException {
		try {
				...
				public String[] mapFileName(String sourceFileName) {
					...
					String cnut = outputDir == null
							? ((srcFile.getParent() == null ? "" : (srcFile.getParent() + File.separator)) + basepath
									+ ".cnut")
							: (new File(outputDir, basepath + ".cnut").getPath());
			...
			for (String src : compileList) {
				String out = mapper.mapFileName(src)[0];
				log("Compiling " + src + " to " + out) ;
				if (!Sq.get().compile(src, out, baseDir) && failOnError) {
