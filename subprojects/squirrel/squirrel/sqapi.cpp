...
#if SQ_NATIVE_DEBUGHOOK

SQDEBUGHOOK sq_getdebughook(HSQUIRRELVM v)
{
	return v->_debughook;
}

void sq_setdebughook(HSQUIRRELVM v, SQDEBUGHOOK hook)
{
	v->_debughook = hook;
}
...
SQInteger buf_read(SQUserPointer bufState,SQUserPointer buf,SQInteger size)
{
//	SQInteger ret;

	BufState *bs = (BufState*)bufState;

	if( bs->ptr >= bs->size )
		return -1;

	int nread;
	if( bs->size - bs->ptr < size )
		nread = bs->size - bs->ptr;
	else
		nread = size;

	memcpy(buf, bs->buf + bs->ptr, nread);
...
SQRESULT sq_compilebuffer(HSQUIRRELVM v,const SQChar *s,SQInteger size,const SQChar *sourcename,SQBool raiseerror) {
	...
	//Check to see if it is a compiled squirrel .nut file, if so dont need to compile the data
	//just load it in
	if(tag == SQ_BYTECODE_STREAM_TAG)
	{
		//BYTECODE, reading cnut compiled code
		if(SQ_SUCCEEDED(sq_readclosure(v,buf_read,&buf)))
		{
			return SQ_OK;
		}
	}
