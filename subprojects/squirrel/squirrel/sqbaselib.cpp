...
static SQInteger base_setdebughook(HSQUIRRELVM v)
{
	...
	// base_setdebughook doesn't make sense in this context, so do nothing
#else
	sq_setdebughook(v);
#endif
	return 0;
...
static SQInteger string_slice(HSQUIRRELVM v)
{
	...
	if(eidx > slen) {
		...
		/* TODO
		 *
		 * This is a work around for a bug in the EarthEternal scripting code (possibly highlighted
		 * because string.slice() NOW has bounds checking, where it didn't before?
		 *
		 * Ideally this needs to be fixed in the EE squirrel scripting code (InputArea.nut.getCursorInRange(), the cursor
		 * position calculation doesn't take into account empty strings.
		 *
		 * There may be other instances of this bug too so beware!
		 */
		 //return sq_throwerror(v,_SC("slice out of range"));
	}
